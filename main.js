...
class Api
{
    ...
    /**
     * получает все параметры строчки запроса
     *
     * @author        User Name
     * @version       v.1.0 (dd/mm/yyyy)
     * @return        {array}
     ...
     */
    get_string_params(string)
    {
        return [...string.matchAll(/\%(.*?)\%/g)].map(el => el[1]);
    }
    ...
    {
        const params = this.get_string_params(template);

        return params.reduce( (acc, val) => {
                const regex = new RegExp(`%${val}%`);

                return acc.replace(regex, encodeURIComponent(object[val]));
            }, template)
